...
```{r}
#| code-fold: true
#| code-summary: "Show one solution"
#| eval: false
...
# 1.  Are bands more successful than solo artists?
rolling_stone[, band := fifelse(artist_member_count == 1, "solo", "band")] |> 
  _[, .(mean_rank_2003 = mean(rank_2003, na.rm = TRUE),
        mean_rank_2012 = mean(rank_2012, na.rm = TRUE),
        mean_rank_2020 = mean(rank_2020, na.rm = TRUE)), by = band]
...
The `by` can also accepts a list so we'll use the `.()` alias.
...
- [] Revisar ejercicio de melt y solución
- [] Agregar cajita de nota con dcast
- [] Agregar .SD? Capaz correlación entre spotify y raking?