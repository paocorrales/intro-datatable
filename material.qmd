...
data.table comes with a function to read regular delimited files such as csv files.
As you may imagine the f in `fread()` means *fast*, this is because the function detects automatically the delimiter, the class of each collumn and the number of rows in the file.
...
But sometimes you'll have a data.frame or a tiblle.
For those cases you can convert them to data.table with `setDT()` and the other way around with `setDF()`.
